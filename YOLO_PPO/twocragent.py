...
import numpy as np
from YOLO_PPO.agents import *
from YOLO_PPO.agents import CRPPOAgent
from UTILS.other_helpers import Elixir
from UTILS.tower_health import TowerHealth
from UTILS.yolo_helper import YoloModel
...
class CRAgent():
    ...
    def getAction(self, observation, mask1=None, mask2=None):
        """
        ...
        """
        return self.actor.get_action(observation, mask1, mask2)
...
#     "height": 250
# }
# # For the second agent's health bars, shift the "left" by left_offset:
# health_bars_2 = []
# for hb in health_bars_1:
#     new_hb = dict(hb)  # shallow copy
#     new_hb["left"] = hb["left"] + left_offset
#     health_bars_2.append(new_hb)
# print(health_bars_2)
...
troop_viewer_1 = YoloModel(model_path="./weights/game/best.pt", img_size=600, deck=False)
deck_viewer_1  = YoloModel(model_path="./yolov5/runs/train/exp18/weights/best.pt", img_size=200, deck=True)
tower_viewer_1 = TowerHealth(level=9)
elixir_viewer_1 = Elixir()

# Common YOLO models for agent 2 (has its own instance):
troop_viewer_2 = YoloModel(model_path="./weights/game/best.pt", img_size=600, deck=False)
deck_viewer_2  = YoloModel(model_path="./yolov5/runs/train/exp18/weights/best.pt", img_size=200, deck=True)
tower_viewer_2 = TowerHealth(level=9)
elixir_viewer_2 = Elixir()

# Construct Agent #1:
obs_dim_1 = troop_viewer_1.getDim() + deck_viewer_1.getDim() + tower_viewer_1.getDim() + 2
agent1 = CRAgent(obs_dim_1)
# agent1.actor.load_model()

# Construct Agent #2 with the appropriate observation dimension:
obs_dim_2 = troop_viewer_2.getDim() + deck_viewer_2.getDim() + tower_viewer_2.getDim() + 2
...
mask1_seq_1     = torch.zeros((sequence_len, 5))
...
mask1_seq_2     = torch.zeros((sequence_len, 5))
...
while True:
    ...
    if contact1.inbattle and searching:
        ...
        troops_1 = troop_viewer_1.predict(troop_frame_1)
        ...
        if ind_1 > 0 and ind_1 % sequence_len == 0:
            ...
            cur_ad_1 = agent1.actor.calculate_advantage(
                obs_seq_1, next_obs_seq_1, reward_seq_1, done_seq_1
            )
        ...
        deckmask1_1 = np.array([1, deck_1[0], deck_1[4], deck_1[8], deck_1[12]])
        deckmask2_1 = []
        deckmask2_1.append(agent1.getBoardMask("troop_noaction"))
        for i in range(4):
            # Example logic to pick board action
            if deck_1[i*4 + 3] == 1 and deck_1[i*4 + 2] == 0 and deck_1[i*4 + 1] == 0:
                deckmask2_1.append(agent1.getBoardMask("spell"))
            else:
                # ...some logic based on tower health
                if tower_viewer_1.getAllHealth()[0] == 0:
                    if tower_viewer_1.getAllHealth()[1] == 0:
                        deckmask2_1.append(agent1.getBoardMask("troop_b"))
                    else:
                        deckmask2_1.append(agent1.getBoardMask("troop_l"))
                elif tower_viewer_1.getAllHealth()[1] == 0:
                    deckmask2_1.append(agent1.getBoardMask("troop_r"))
                else:
                    deckmask2_1.append(agent1.getBoardMask("troop_n"))

        action_1, log_prob_1, m1_1, m2_1 = agent1.getAction(obs_1, mask1=deckmask1_1, mask2=deckmask2_1)

        # If there's a non-zero action in the first dimension, place troop, etc.
        if action_1[0] > 0:
            reward_1 -= 1
            troop_num_1 = 4*deck_1[(action_1[0]-1) * 4 +1] \
                          +2*deck_1[(action_1[0]-1) * 4 +2] \
                          + deck_1[(action_1[0]-1) * 4 +3]
            x_1 = action_1[1] % 18
            y_1 = action_1[1] // 18
    ...
    if contact2.inbattle and searching:
        ...
        # YOLO predictions for troops & deck
        troops_2 = troop_viewer_2.predict(troop_frame_2)
        ...
        deckmask1_2 = np.array([1, deck_2[0], deck_2[4], deck_2[8], deck_2[12]])
        deckmask2_2 = []
        deckmask2_2.append(agent2.getBoardMask("troop_noaction"))
        for i in range(4):
            if deck_2[i*4 + 3] == 1 and deck_2[i*4 + 2] == 0 and deck_2[i*4 + 1] == 0:
                deckmask2_2.append(agent2.getBoardMask("spell"))
            else:
                if tower_viewer_2.getAllHealth()[0] == 0:
                    if tower_viewer_2.getAllHealth()[1] == 0:
                        deckmask2_2.append(agent2.getBoardMask("troop_b"))
                    else:
                        deckmask2_2.append(agent2.getBoardMask("troop_l"))
                elif tower_viewer_2.getAllHealth()[1] == 0:
                    deckmask2_2.append(agent2.getBoardMask("troop_r"))
                else:
                    deckmask2_2.append(agent2.getBoardMask("troop_n"))

        action_2, log_prob_2, m1_2, m2_2 = agent2.getAction(obs_2, mask1=deckmask1_2, mask2=deckmask2_2)

        # If there's a non-zero action in the first dimension, place troop, etc.
        if action_2[0] > 0:
            reward_2 -= 1
            troop_num_2 = 4*deck_2[(action_2[0]-1) * 4 +1] \
                          + 2*deck_2[(action_2[0]-1) * 4 +2] \
                          + deck_2[(action_2[0]-1) * 4 +3]
            x_2 = action_2[1] % 18
            y_2 = action_2[1] // 18
